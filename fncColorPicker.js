...
var fncColorPicker =
{
	...
	Create: function(config)
	{
		...
		this.angleToBasis = function(x, y)
		{
			...
			console.log("raw angle_to_basis: " + angle_to_basis);
		...
		this.$container_div.mousemove( function(event) {
			...
			if (this.isOverColorWheel(coords.x,coords.y))
			{
				...
				console.log("angle_to_basis: " + angle_to_basis);
		...
		const x_center = this.container_size / 2.0;
		const y_center = this.container_size / 2.0;
		...
		// ---- RENDER ----
		for (var cur_slice = 0; cur_slice < num_slices; cur_slice += 1)
		{
			...
			ctx.arc(	this.container_size/2, this.container_size/2,
						this.wheel_radius, 
						this.rot_offset + (cur_slice * slice_sweep_rads),
						this.rot_offset + ((cur_slice+1) * slice_sweep_rads + 0.01), // little extra to cover gaps 
						false);
			// ctx.strokeStyle = gradient;
			ctx.strokeStyle = `hsl(${cur_slice * slice_sweep_degrees},100%,50%)`;
			ctx.lineWidth = this.wheel_stroke_width; //30;	
			ctx.lineCap = 'butt';
			ctx.stroke();
		}	// end for cur_slice
